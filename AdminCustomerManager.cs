...
// /​
//My application uses C# collections and LINQ to ensure efficient data handling.​
//Customers are stored in a Dictionary<string, Customer> for quick O(1) lookups, avoiding duplicates and scaling well.​
//Each customer’s rental history is a List<string>, allowing fast appending and maintaining order for upgrade logic​
// , exposed as IReadOnlyList<string> to prevent external changes.​
//In admin mode, LINQ methods like Where, SelectMany, GroupBy, OrderByDescending, and Take() efficiently filter data and generate statistics, replacing slow loops.​
//Dictionaries are preferred over Lists for customer storage due to faster key-based access.​
//Improvements include​
//replacing dictionaries with indexed database storage,​
//adding ConcurrentDictionary for thread safety​
//, caching frequent customers,​
//using memory-efficient structures for large datasets,​
//and indexing to optimise LINQ queries.​
//​
//A ConcurrentDictionary is a thread-safe version of a normal Dictionary, allowing multiple threads to read and write data simultaneously without crashes or data corruption. Currently, my app uses small data amounts and runs mainly on one thread, so a normal Dictionary suffices. A ConcurrentDictionary becomes beneficial in larger, multi-threaded systems, like servers or high-load applications, where many users access data concurrently.

// This is exposed using IReadOnlyList<string> to protect encapsulation and prevent external modification.​
//In admin mode, LINQ algorithms such as Where, SelectMany, GroupBy, OrderByDescending, and Take() are used to efficiently filter data and generate statistics like the most rented cars.​
// These replace slow manual loops and keep the code clean and performant.​
//A Dictionary was chosen over a List for customer storage because lists require O(n) searching through every element, whereas dictionaries allow direct key-based access.​
// This code uses IReadOnlyList<string> for encapsulation and uses LINQ methods like Where, SelectMany, GroupBy, OrderByDescending, and Take() for efficient data filtering and statistics.It replaces slow loops to keep the code clean and fast.A Dictionary is used over a List for customer data because dictionaries provide quicker, key - based access.Suggested improvements include replacing in-memory dictionaries with indexed database storage for scalability, adding ConcurrentDictionary for thread safety, caching frequently accessed customers, using memory-efficient structures for large datasets, and implementing indexing to speed up LINQ queries.
//​
//A ConcurrentDictionary is a thread-safe version of a normal Dictionary. It allows multiple parts of the program (threads) to read and write data at the same time without causing crashes or corrupting the data.​
//Right now my app uses small amounts of data and mostly runs on a single thread, so a normal Dictionary is fine. A ConcurrentDictionary becomes useful when the application scales up and uses multiple threads, like in servers or high-load systems This would be useful in a larger, multi-threaded version of the application where many users are accessing data at the same time.
